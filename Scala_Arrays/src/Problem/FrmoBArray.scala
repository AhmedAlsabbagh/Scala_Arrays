object FrmoBArray {
  def main(args: Array[String]): Unit = {
    ...
    // To print all the elements of an array using option 1: for loop and option 2: foreach
    // Option no 1: using for loop
    for (mark <- marks) {     // Accessing each elements of an array marks
      println(mark)
    }

    // Option no 2: Using foreach
    marks.foreach(println)

  // Find the average marks using option 1: for loop and option 2: foreach
    // Option no 1: Using for loop
    var totalMarks = 0
    ...
    // Option no 2: Using foreach loop
    totalMarks = 0
